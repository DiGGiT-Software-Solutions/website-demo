...
! function() {
    ...
    function o(t, r, i) {
        function o(t, r) {
            ...
            if (e.shaderSource(i, r), e.compileShader(i), !e.getShaderParameter(i, e.COMPILE_STATUS)) throw new Error("compile error: " + e.getShaderInfoLog(i));
            return i
        }
        var n = {};
        if (n.id = e.createProgram(), e.attachShader(n.id, o(e.VERTEX_SHADER, t)), e.attachShader(n.id, o(e.FRAGMENT_SHADER, r)), e.linkProgram(n.id), !e.getProgramParameter(n.id, e.LINK_STATUS)) throw new Error("link error: " + e.getProgramInfoLog(n.id));
        n.uniforms = {}, n.locations = {}, e.useProgram(n.id), e.enableVertexAttribArray(0);
        for (var a, s, u = /uniform (\w+) (\w+)/g, h = t + r; null != (a = u.exec(h));) s = a[2], n.locations[s] = e.getUniformLocation(n.id, s);
        return n
    }

    function n(t, r) {
        e.activeTexture(e.TEXTURE0 + (r || 0)), e.bindTexture(e.TEXTURE_2D, t)
    }
    ...
    var s = function() {
            ...
            if (!(e = t.getContext("webgl") || t.getContext("experimental-webgl"))) return null;
            var r = {};
            ...
            i.push(o("float", e.FLOAT, Float32Array)), r.OES_texture_half_float && i.push(o("half_float", r.OES_texture_half_float.HALF_FLOAT_OES, null));
            var n = e.createTexture(),
                a = e.createFramebuffer();
            e.bindFramebuffer(e.FRAMEBUFFER, a), e.bindTexture(e.TEXTURE_2D, n), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, e.NEAREST), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MAG_FILTER, e.NEAREST), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, e.CLAMP_TO_EDGE), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, e.CLAMP_TO_EDGE);
            for (var s = null, u = 0; u < i.length; u++)
                if (e.texImage2D(e.TEXTURE_2D, 0, e.RGBA, 32, 32, 0, e.RGBA, i[u].type, null), e.framebufferTexture2D(e.FRAMEBUFFER, e.COLOR_ATTACHMENT0, e.TEXTURE_2D, n, 0), e.checkFramebufferStatus(e.FRAMEBUFFER) === e.FRAMEBUFFER_COMPLETE) {
                    s = i[u];
                    break
                }
            return s
    ...
    var h = function(r, i) {
        var o = this;
        this.$el = t(r), this.interactive = i.interactive, this.resolution = i.resolution, this.textureDelta = new Float32Array([1 / this.resolution, 1 / this.resolution]), this.perturbance = i.perturbance, this.dropRadius = i.dropRadius, this.crossOrigin = i.crossOrigin, this.imageUrl = i.imageUrl;
        var n = document.createElement("canvas");
        n.width = this.$el.innerWidth(), n.height = this.$el.innerHeight(), this.canvas = n, this.$canvas = t(n), this.$canvas.css({
            position: "absolute",
            left: 0,
            top: 0,
            right: 0,
            bottom: 0,
            zIndex: -1
        }), this.$el.addClass("jquery-ripples").append(n), this.context = e = n.getContext("webgl") || n.getContext("experimental-webgl"), s.extensions.forEach(function(t) {
            e.getExtension(t)
    ...
    h.DEFAULTS = {
            ...
            if (r != this.imageSource)
                if (this.imageSource = r, this.imageSource) {
                    ...
                    i.onload = function() {
                        ...
                        e = t.context;
                        var o = r(i.width) && r(i.height) ? e.REPEAT : e.CLAMP_TO_EDGE;
                        e.bindTexture(e.TEXTURE_2D, t.backgroundTexture), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, o), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, o), e.texImage2D(e.TEXTURE_2D, 0, e.RGBA, e.RGBA, e.UNSIGNED_BYTE, i), t.backgroundWidth = i.width, t.backgroundHeight = i.height, t.hideCssBackground()
                    }, i.onerror = function() {
                        e = t.context, t.setTransparentTexture()
                    }, i.crossOrigin = this.imageSource.match(/^data:/) ? null : this.crossOrigin, i.src = this.imageSource
                } else this.setTransparentTexture()
        },
        step: function() {
            e = this.context, this.visible && (this.computeTextureBoundaries(), this.running && this.update(), this.render())
        },
        drawQuad: function() {
            e.bindBuffer(e.ARRAY_BUFFER, this.quad), e.vertexAttribPointer(0, 2, e.FLOAT, !1, 0, 0), e.drawArrays(e.TRIANGLE_FAN, 0, 4)
        },
        render: function() {
            e.bindFramebuffer(e.FRAMEBUFFER, null), e.viewport(0, 0, this.canvas.width, this.canvas.height), e.enable(e.BLEND), e.clear(e.COLOR_BUFFER_BIT | e.DEPTH_BUFFER_BIT), e.useProgram(this.renderProgram.id), n(this.backgroundTexture, 0), n(this.textures[0], 1), e.uniform1f(this.renderProgram.locations.perturbance, this.perturbance), e.uniform2fv(this.renderProgram.locations.topLeft, this.renderProgram.uniforms.topLeft), e.uniform2fv(this.renderProgram.locations.bottomRight, this.renderProgram.uniforms.bottomRight), e.uniform2fv(this.renderProgram.locations.containerRatio, this.renderProgram.uniforms.containerRatio), e.uniform1i(this.renderProgram.locations.samplerBackground, 0), e.uniform1i(this.renderProgram.locations.samplerRipples, 1), this.drawQuad(), e.disable(e.BLEND)
        },
        update: function() {
            e.viewport(0, 0, this.resolution, this.resolution), e.bindFramebuffer(e.FRAMEBUFFER, this.framebuffers[this.bufferWriteIndex]), n(this.textures[this.bufferReadIndex]), e.useProgram(this.updateProgram.id), this.drawQuad(), this.swapBufferIndices()
        },
        ...
        initTexture: function() {
            this.backgroundTexture = e.createTexture(), e.bindTexture(e.TEXTURE_2D, this.backgroundTexture), e.pixelStorei(e.UNPACK_FLIP_Y_WEBGL, 1), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MAG_FILTER, e.LINEAR), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, e.LINEAR)
        },
        setTransparentTexture: function() {
            e.bindTexture(e.TEXTURE_2D, this.backgroundTexture), e.texImage2D(e.TEXTURE_2D, 0, e.RGBA, e.RGBA, e.UNSIGNED_BYTE, u)
        },
        ...
        drop: function(t, r, i, o) {
            e = this.context;
            var a = this.$el.innerWidth(),
                s = this.$el.innerHeight(),
                u = Math.max(a, s);
            i /= u;
            var h = new Float32Array([(2 * t - a) / u, (s - 2 * r) / u]);
            e.viewport(0, 0, this.resolution, this.resolution), e.bindFramebuffer(e.FRAMEBUFFER, this.framebuffers[this.bufferWriteIndex]), n(this.textures[this.bufferReadIndex]), e.useProgram(this.dropProgram.id), e.uniform2fv(this.dropProgram.locations.center, h), e.uniform1f(this.dropProgram.locations.radius, i), e.uniform1f(this.dropProgram.locations.strength, o), this.drawQuad(), this.swapBufferIndices()
        },
        updateSize: function() {
    ...
    t.fn.ripples = function(e) {
        ...
        return this.each(function() {
            var i = t(this),
                o = i.data("ripples"),
                n = t.extend({}, h.DEFAULTS, i.data(), "object" == typeof e && e);
            (o || "string" != typeof e) && (o ? "string" == typeof e && h.prototype[e].apply(o, r) : i.data("ripples", o = new h(this, n)))
        })
    }, t.fn.ripples.Constructor = h, t.fn.ripples.noConflict = function() {
